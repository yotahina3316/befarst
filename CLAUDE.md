...
- **不具合2: メンバーの誕生日がSCHEDULEカレンダーに反映されていない**。原因は`backend/befarst/recurring_events.py`の`_next_occurrence()`が「今日以降でもっとも近い1回分の日付」しか生成しない設計だったこと。今年の誕生日をすでに迎えたメンバー(2026-09-23時点でSOTA/MANATO/JUNON/SHUNTO/LEOの5人が該当)は来年の日付でしか予定が存在せず、カレンダーで今年の該当月を開いても(すでに過ぎているため当然だが)何も表示されず、「反映されていない」ように見えていた。
  ...
  - **上記修正直後に発覚した副作用(2026-09-23、同日中に追加修正)**: 当年・翌年を無条件に両方生成すると、当年分の日付がまだ来ていないメンバー(例: 11月生まれのRYUHEI、11/3のデビュー記念日は9月時点ではまだ「今後の予定」)については当年・翌年の両方が同時に「未来の予定」に該当してしまい、MEMBERページの予定一覧やSCHEDULEカレンダーに同じ予定が2件重複して表示される不具合が出た(デビュー記念日がMEMBERページに2回表示される形でユーザーが発見)。`_occurrences()`を「翌年分は当年分がすでに過ぎている場合にのみ追加する」ロジックに修正し、「今後の予定」が常に1件だけになるようにした。誤って生成済みだった`RYUHEI-2027`・`debut-2027`の2件は、当時点でまだ当年分(`RYUHEI-2026`・`debut-2026`)が未来の予定として残っていたため不要と判断し、`docs/data/schedule.json`から直接削除して修正した。
  - **同時に対応: カード内に誕生日/記念日の日付そのものが表示されていなかった**。`summary_ja`が「SOTAの誕生日です。」のような固定文言のみで、具体的な月日が書かれていなかったため、カードを見ても本人の誕生日がいつなのか分からなかった(ユーザー指摘、2026-09-23)。`_build_item()`の`summary_ja`を`"{month}月{day}日です。"`のように実際の開催日から生成する形に変更し、既存データ(`docs/data/schedule.json`内の誕生日/記念日アイテム)についても同じ形式に一括更新した。
